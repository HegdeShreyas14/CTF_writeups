...
The php website allows us to upload files which have .txt extension and from the code we can see that on uplod chmod 000 runs which makes the file inaccessible. 
- the flag file is under /uploads/flag.txt , trying to accessing it directly throws a 403 forbidden error which on looking at the dockerfile made it clear as chmod 000 was run on it as well
- Searching the web for arguments and file names which would allow me to bypass the .txt condition and run the command to hopefully change the permissions led me to --reference option
- `shell_exec('chmod 000 *');`  means that the * is expanded to the shell with filenames as arguments instead of plain file name
- --reference=file applies permissions of reference file to all the files 
![alt text](img1.png)
