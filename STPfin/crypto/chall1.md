# Challenge 1
 Given in the handout:
``` python
from secret import a,b,p,flag
from hashlib import sha512
from pwn import xor
from Crypto.Util.number import isPrime

assert p == a**51 + 51*b**51 and isPrime(p) and a > 0 and b > 0
print(hex(p)[2:])
print(xor(flag,sha512((str(a)+str(b)).encode()).digest()).hex())

# 
# 43edcf6275293ce97d716f49875c4bdba37f6ab30f15a53f09b72bf3816edf6b92618fb56d569d911b2f6fe7a36d4a854022dddf671dc89b4800bc1605822aab72d3
```

- The flag seems to be encrypted by performing XOR with SHA512 hash of concatenated string of a and b
- On rewriting the equation by taking the mod of p we get 

`0=a^51+51b^51 (mod p)`  
This can be further written as 

`a=((-51)^(1/51))(b)(mod p)`  
The numeric part is a constant so we have `a=l*b` where l is the constant numeral part